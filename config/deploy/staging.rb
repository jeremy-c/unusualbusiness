set :stage, :staging
set :stage_application, "s-#{fetch(:application)}"
...
set :wpcli_remote_url, -> {"http://#{fetch(:application)}.linksmith.nl"}
#Url of the Wordpress root installation on the remote server (used by search-replace command).

set :wpcli_local_url, -> {"http://#{fetch(:application)}.nl.dev"}
...
#set :local_tmp_dir, -> { "/var/www/vhosts/d-#{fetch(:application)}" }
#A local temp dir which is read and writeable. Defaults to /tmp.
